...
**v0.3.0 update:** The IT workflow now uses LangGraph with durable SQLite checkpoints and human-review interrupts. See [LangGraph release scope](docs/LANGGRAPH_RELEASE.md). Invoice/infrastructure and other enterprise extensions are still pending; this is not the complete challenge platform.
...
- No cloud purchase, signup, email, challenge submission, git staging/commit or push performed.
- Preserve the existing Docker installation; do not reset/purge other workloads.
- A supported enterprise operating model, real source/security connectors, enterprise identity, legal/security review, resilience and workload testing remain required before ACG deployment. No no-code authoring, invoice/infrastructure workflow, production writes, certifications or 500-user claim.
