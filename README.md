...
- **Injection judge:** every run that reaches review is screened by the same local model. The ticket and each cited source are judged separately (at most 1 + `MAX_SOURCES` items; the rest are shown as not screened) and the judge may only return an enum verdict and signal list, no free text. The result is stored on the run, bound into the approval digest and shown on the review card; approving a flagged action needs an extra confirmation. It is advisory only: it never blocks, approves or executes, a 1.5B model can miss injections or be manipulated itself, and judge failures show as "not screened". Invoice and infrastructure runs now make judge calls; their calculations remain deterministic. The fake test model flags only the phrase "ignore previous instructions".
